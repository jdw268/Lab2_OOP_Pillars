...
* Class: CIT-244
* Date:  9.18.17
* Description: This class is the Truck class that extends the Vehicle class
* It's used to create a Truck object.
*/
import java.io.*;
import java.util.*;
...
public class Truck extends Vehicle{
	...
	public Truck(String makeInput, String modelInput, FileReader configInput, int bedSizeInput, int engineInput){
		//set inherited data with super
	...
	public int getBedSize(){
		...
		String bedSizeString = null;
		int bedSizeInt;

		//search customOption in config file for:  search from truck and 5 indexes of list
		//find index of "truck" first
		while(continueRead){
			...
			if (line.equalsIgnoreCase("Truck")) {
				continueRead = false;
			}
		...
		//now in Truck options; find bed size
		continueRead = true;
		while(continueRead){
		...
		bedSizeInt = Integer.parseInt(bedSizeString);
	...
	*/
	public String readModelName(){
		...
		//search the name for substring "Truck" and add 5 for other options
		position = modelName.indexOf("Truck") + 5;
